...
export default function WriteReview({ hotel, onSuccessToast }: WriteReviewProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const targetHotelId = hotelId || hotel?.id;
        if (!targetHotelId) {
            alert('ID hotel tidak ditemukan. Tidak bisa mengirim ulasan.');
            return;
        }
        ...
        try {
            await apiClient.post('/reviews', {
                ...
                ratingKebersihan: ratings.kebersihan,
                ratingPelayanan: ratings.pelayanan,
                ratingLokasi: ratings.lokasi,
                ratingFasilitas: ratings.fasilitas,
                comment,
                images,
            });
            onSuccessToast(`Ulasan Anda untuk ${hotel?.name || 'hotel'} berhasil dikirim! Terima kasih.`);
            navigate('/dashboard');
        } catch (err: any) {
            console.error('Gagal kirim ulasan:', err);
            alert(err?.message || 'Gagal mengirim ulasan. Silakan coba lagi.');
        } finally {
    ...
    return (
        <main
            className="max-w-3xl mx-auto px-4 md:px-8 py-8 text-left font-sans flex-grow w-full pb-32 animate-in fade-in slide-in-from-bottom-4 duration-500 ease-out fill-mode-forwards"
            ...
            <div className="bg-white dark:bg-[#1A2B4C] rounded-[24px] border border-gray-150 dark:border-gray-800 p-6 md:p-8 shadow-sm">
                ...
                <form onSubmit={handleSubmit} className="space-y-8 text-left">
                    ...
                    <button
                        ...
                        className="w-full h-[56px] bg-[#1A2B4C] hover:bg-[#031636] dark:bg-[#fed023] dark:hover:bg-[#eec209] text-white dark:text-[#031636] rounded-full font-bold text-sm tracking-wide shadow-md active:scale-[0.98] transition-all flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed"
                    >
                        {isSubmitting ? 'Mengirim...' : <><span>Kirim Ulasan</span> <CheckCircle className="w-4 h-4" /></>}
                    </button>
                </form>
            </div>
        </main>

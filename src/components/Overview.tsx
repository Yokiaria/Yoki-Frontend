...
export default function Overview({
  ...
  // Compute numbers based on bookings
  const totalRooms = rooms.length;
  const occupiedCount = bookings.filter((b) => 
    String(b.status).toUpperCase() === 'CHECK_IN' || 
    String(b.status).toUpperCase() === 'CLAIMED'
  ).length;
  const activeCount = bookings.filter((b) => ['pending', 'aktif', 'check_in', 'dibayar', 'booked', 'CHECKED_IN', 'CLAIMED', 'BOOKED'].includes(String(b.status))).length;
  ...
  const FormatRupiahAnimated = ({ num }: { num: number }) => (
    <>Rp <AnimatedCounter targetValue={num} /></>
  );
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        >
          ...
          <div>
            ...
            <h3 className="font-display text-4xl font-extrabold text-primary">{occupiedCount} <span className="text-sm font-normal text-on-surface-variant">dari {totalRooms} unit</span></h3>
          </div>
        ...
        >
          ...
          <div>
            ...
            <h3 className="font-display text-4xl font-extrabold text-primary">
              <AnimatedCounter targetValue={activeCount} /> <span className="text-sm font-normal text-on-surface-variant">Unit</span>
            </h3>
          </div>
        ...
        >
          ...
          <div>
            ...
            <h3 className="font-display text-4xl font-extrabold text-primary">{availableCount} <span className="text-sm font-normal text-on-surface-variant">unit</span></h3>
          </div>
          <div className="absolute bottom-0 left-0 w-full h-1 bg-primary scale-x-0 group-hover:scale-x-100 origin-left transition-transform duration-300"></div>
        </div>
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <h3 className="font-display text-3xl font-extrabold text-white">
              <FormatRupiahAnimated num={revenueToday} />
            </h3>
          </div>
        </div>
      </div>

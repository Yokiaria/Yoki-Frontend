...
interface RoomTypesProps {
  ...
  roomTypes: RoomType[];
  rooms: Room[];
  onSaveRoomType: (newType: RoomType, isEdit: boolean) => void;
...
export default function RoomTypes({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!formHotelId || !formName || !formPrice) {
      alert('Mohon isi kolom wajib (*) terlebih dahulu.');
      return;
    }

    const newRoomType: RoomType = {
      id: formId || ('rt-' + Date.now()),
      hotelId: formHotelId,
      name: formName,
    ...
    onSaveRoomType(newRoomType, !!formId);
    handleResetForm();
  };
          ...
          <section className="glass-panel rounded-24 p-6 md:p-8 shadow-sm border border-outline-variant/20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="col-span-1 md:col-span-2">
                <label className="block text-xs font-bold tracking-wider uppercase text-on-surface-variant mb-2" htmlFor="hotel_id">
                ...
                {hotels.length > 10 ? (
                  ...
                  <select
                    id="hotel_id"
                    className="w-full bg-white px-4 py-3 rounded-xl border border-outline-variant text-on-surface font-medium focus:ring-2 focus:ring-primary/20 transition-all outline-none appearance-none"
                    required
                    value={formHotelId}
              ...
              <tbody className="divide-y divide-outline-variant/10">
                {filteredRoomTypes.map((type) => (
                  <tr key={type.id} className="hover:bg-surface-bright transition-colors group">
                    ...
                    {/* Hotel */}
                    <td className="p-6">
                      <p className="font-semibold text-on-surface text-sm">{type.hotel?.name || type.hotelId || '-'}</p>
                    </td>

                    {/* Amenities / Facilities */}
                    <td className="p-6">
                      <div className="flex flex-wrap gap-1.5 max-w-[280px]">
                        {type.facilities.slice(0, 4).map((f: any) => {
                          const facilityId = typeof f === 'string' ? f : f?.facility;
                          const keyId = typeof f === 'string' ? f : (f?.id || f?.facility);
                          return (
                          <span
                            key={keyId}
                          ...
                          >
                            <span className="material-symbols-outlined text-[13px]">{getFacilityIcon(facilityId)}</span>
                            {getFacilityLabel(facilityId)}
                          </span>
                        )})}
                    ...
                    <td className="p-6 text-center">
                      <div className="flex items-center justify-center gap-2 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                          onClick={() => {
                            setFormId(type.id);
                            setFormHotelId(type.hotelId || '');
                            if (type.hotelId) {
                                const h = hotels.find(x => x.id === type.hotelId);
                                if (h) setHotelSearchQuery(h.name);
                            } else {
                                setHotelSearchQuery('');
                            }

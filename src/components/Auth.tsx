...
export default function Auth({ isLogin, setUser, onSuccessToast, onErrorToast }: { isLogin: boolean, setUser: any, onSuccessToast: any, onErrorToast: any }) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            let data: any;
            if (isLogin) {
                data = await apiClient.post('/auth/login', { email, password });
            } else {
                data = await apiClient.post('/auth/register', { name, email, password, phone });
            }

            // Simpan token dengan key gsi_token
            if (data.access_token || data.token) {
                localStorage.setItem('gsi_token', data.access_token || data.token);
            }

            // Simpan data user ke localStorage
            const userData = data.user || data;
            localStorage.setItem('gsi_user', JSON.stringify(userData));
            setUser({
                name: userData.name || (name || 'Tamu GrandStarInd'),
                email: userData.email || email,
                phone: userData.phone || '+62 811 0000 0000',
                loyaltyPoints: userData.loyaltyPoints || 1250,
                tier: userData.tier || 'Regular Member',
                role: userData.role || 'user',
                balance: userData.balance ? Number(userData.balance) : 0,
            });
            
            onSuccessToast(isLogin ? `Selamat datang kembali, ${userData.name || 'User'}!` : 'Registrasi Berhasil! Selamat datang di GrandStarInd.');
            navigate('/');
        } catch (error: any) {
            if (onErrorToast) {
                onErrorToast(error.message || 'Terjadi kesalahan saat autentikasi');
            } else {
    ...
    return (
        <div className="flex-grow flex items-center justify-center p-6 md:p-12 relative overflow-hidden bg-[#f7f9fb] dark:bg-[#031636] animate-in fade-in slide-in-from-bottom-4 duration-500 ease-out fill-mode-forwards">
            ...
            <div className="w-full max-w-5xl bg-white dark:bg-[#071b3b] rounded-3xl shadow-2xl flex flex-col md:flex-row overflow-hidden relative z-10 border border-gray-100 dark:border-gray-800 animate-fade-in">
                ...
                <div className="w-full md:w-1/2 p-8 md:p-12 lg:p-16 flex flex-col justify-center">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        {/* Phone field — hanya muncul saat Register */}
                        {!isLogin && (
                            <div className="space-y-1.5">
                                <label className="text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Nomor Telepon</label>
                                <input
                                    type="tel"
                                    value={phone}
                        ...
                        <button 
                            type="submit"
                            disabled={loading}
                            className={`w-full bg-[#031636] dark:bg-[#b6c6f0] hover:bg-[#1a2b4c] dark:hover:bg-white text-white dark:text-[#031636] font-bold py-4 px-4 rounded-xl shadow-lg hover:shadow-xl transition-all mt-4 transform active:scale-95 ${loading ? 'opacity-70 cursor-not-allowed' : ''}`}
                        >
                    ...
                    <div className="mt-8 text-center text-sm font-medium text-gray-600 dark:text-gray-400">
                        {isLogin ? (
                            <p>
                                Belum punya akun?{' '}
                                <button onClick={() => navigate('/register')} className="text-[#031636] dark:text-[#b6c6f0] font-bold hover:underline transition-all">
                                    Daftar di sini
                                </button>
                            </p>
                        ) : (
                            <p>
                                Sudah punya akun?{' '}
                                <button onClick={() => navigate('/login')} className="text-[#031636] dark:text-[#b6c6f0] font-bold hover:underline transition-all">
                                    Masuk di sini
                                </button>
                            </p>
                        )}
